...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("FotoCoinMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("FotoCoinMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("FotoCoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("fotocoin-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        if (fProofOfStake) {
            ...
            if (!pblock->SignBlock(*pwallet)) {
                LogPrintf("FotoCoinMiner(): Signing new block failed \n");
                continue;
            }
        ...
        LogPrintf("Running FotoCoinMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadBitcoinMiner(void* parg)
{
    ...
    try {
        ...
        LogPrintf("ThreadFotoCoinMiner() exception");
    } catch (...) {
        LogPrintf("ThreadFotoCoinMiner() exception");
    }

    LogPrintf("ThreadFotoCoinMiner exiting\n");
}
