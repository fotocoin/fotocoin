...
*Setup instructions for a gitian build of FotoCoin using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the FotoCoin
...
- [Building FotoCoin](#building-fotocoin)
...
Clone the git repositories for FotoCoin and gitian and then checkout the FotoCoin version that you want to build.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/fotocoincoin/fotocoin-core.git
cd fotocoin-core
...
Follow the instructions in [doc/release-process.md](release-process.md) in the fotocoin-core repository
...
To build FotoCoin (for Linux, OSX, and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the fotocoin-core repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/fotocoin-core/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/fotocoincoin/fotocoin-core
...
./bin/gbuild --commit fotocoin=${COMMIT} --url fotocoin=${URL} ../fotocoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit fotocoin=${COMMIT} --url fotocoin=${URL} ../fotocoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit fotocoin=${COMMIT} --url fotocoin=${URL} ../fotocoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/fotocoin-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/fotocoin-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/fotocoin-build.assert
```
...
[fotocoincoin/gitian.sigs](https://github.com/fotocoincoin/gitian.sigs/) repository, or if that's not possible to create a pull
request.
